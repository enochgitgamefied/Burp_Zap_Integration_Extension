...
class BurpExtender(IBurpExtender, ITab):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        self.port_input.setText(self._get_config_setting("zap_port", "8082"))  # Default if not set
    ...
    def _initialize_ui(self):
        try:
            ...
            if self.hardcoded_port:
                self.port_input.setText(self._get_config_setting("zap_port", ""))
    ...
    def _browse_image(self, event):
        chooser = JFileChooser()
        chooser.setDialogTitle("Select Logo Image")
        chooser.setFileSelectionMode(JFileChooser.FILES_ONLY)
        chooser.setAcceptAllFileFilterUsed(False)
        chooser.addChoosableFileFilter(FileNameExtensionFilter("Image Files", ["png", "jpg", "jpeg", "gif"]))
        
        if chooser.showOpenDialog(self.panel) == JFileChooser.APPROVE_OPTION:
            selected_file = chooser.getSelectedFile()
            self.image_path_input.setText(selected_file.getAbsolutePath())
            self._save_config_setting("image_path", selected_file.getAbsolutePath())
            self._load_image(selected_file.getAbsolutePath())

    def _load_image(self, image_path):
        try:
            # Remove existing image components
            for component in self.top_panel.getComponents():
                if isinstance(component, JPanel) and component.getComponentCount() > 0:
                    if isinstance(component.getComponent(0), JLabel) and component.getComponent(0).getIcon() is not None:
                        self.top_panel.remove(component)
            
            image = ImageIcon(image_path)
            image_label = JLabel(image)
            center_panel = JPanel(FlowLayout(FlowLayout.CENTER))
            center_panel.add(image_label)
            self.top_panel.add(center_panel, 0)
            self.top_panel.revalidate()
            self.top_panel.repaint()
            self.log("[+] Image loaded successfully from: {}".format(image_path))
        except Exception as e:
            self.log("[!] Error loading image: {}".format(str(e)))
            self._show_text_title()
    ...
    def update_config(self, event):
        ...
        # Update input fields (optional but ensures no trailing spaces)
    ...
    def scan_complete(self):
        with self.lock:
            ...
            SwingUtilities.invokeLater(lambda: [
                ...
                self.stop_button.setEnabled(False),
                # Don't reset progress here - keep at 100% until next action
            ])
